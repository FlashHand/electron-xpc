...
var XpcUtilityProcess = class {
  ...
  async send(handleName, params) {
    ...
    const semaphore$1 = new semaphore.Semaphore(1);
    semaphore$1.take(() => {
    });
    const taskInfo = { semaphore: semaphore$1, ret: null };
    this.pendingTasks.set(payload.id, taskInfo);
    this.port.postMessage({
      type: XPC_EXEC,
      payload
    });
    await semaphore$1.takeAsync();
...
exports.xpcUtilityProcess = xpcUtilityProcess;
